...
<<<<<<< HEAD
        ...
        asyncio.create_task(deploy_model_async(deployment_id, config, port))
        ...
        return {
            ...
            "port": port,
            "status": "initializing",
            "status_url": f"/api/deployments/{deployment_id}/status"
        }
...
async def deploy_model_async(deployment_id: str, config: DeploymentConfig, port: int):
    """Background task to deploy model"""
    try:
        # Update status: Creating directory
        active_deployments[deployment_id]["status"] = "creating_directory"
        active_deployments[deployment_id]["progress"] = 10
        active_deployments[deployment_id]["message"] = "Creating deployment directory..."
        await broadcast_status(deployment_id)
        
        deployment_dir = MODELS_DIR / f"{config.model_name}_{deployment_id}"
        deployment_dir.mkdir(exist_ok=True)
        
        # Update status: Creating server script
        active_deployments[deployment_id]["status"] = "creating_server"
        active_deployments[deployment_id]["progress"] = 30
...
<<<<<<< HEAD
        ...
        if config.dependencies:
            ...
            active_deployments[deployment_id]["progress"] = 50
            active_deployments[deployment_id]["message"] = f"Installing {len(config.dependencies)} dependencies..."
            await broadcast_status(deployment_id)
            
            await install_dependencies(config.dependencies, deployment_dir)
        
        # Update status: Starting server
        active_deployments[deployment_id]["status"] = "starting_server"
        active_deployments[deployment_id]["progress"] = 70
...
<<<<<<< HEAD
        ...
        active_deployments[deployment_id]["message"] = "Verifying server health..."
        active_deployments[deployment_id]["pid"] = process.pid
        active_deployments[deployment_id]["process"] = process
        await broadcast_status(deployment_id)
        ...
        if process.poll() is None:
            ...
            active_deployments[deployment_id]["progress"] = 100
            active_deployments[deployment_id]["message"] = "Deployment successful"
            await broadcast_status(deployment_id)
        ...
        active_deployments[deployment_id]["progress"] = 0
        active_deployments[deployment_id]["message"] = f"Deployment failed: {str(e)}"
...
async def broadcast_status(deployment_id: str):
    ...
    if deployment_id in active_deployments:
        await broadcast_message({
            "type": "status_update",
            "deployment_id": deployment_id,
            "status": active_deployments[deployment_id]["status"],
...
<<<<<<< HEAD
        ...
        "endpoint": f"http://localhost:{dep_info['port']}" if dep_info["status"] == "running" else None,
